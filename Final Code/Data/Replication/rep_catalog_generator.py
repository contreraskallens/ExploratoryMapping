#This script produces the catalog of REPLICATION data. takes the various .txt of abstracts and their respective theories specified in rep_file_key.txt and produces one big catalog of papers, assigning each an ID. Outputs it to 'rep_catalog.txt'#


cat_files = open('rep_file_key.txt', 'r').read().split('\n') #opens files, transforms into strings, splits new lines
output = open('rep_catalog.txt', 'w')
...
for fl in cat_files: #loop in stringed filekey
    ...
    article_dictionary = {} #creates dictionary for each article. Rest of the loop fills that dictionary with the data for each article.
    ...
    for line in file_lines:   #loop in list of every line of every file
        if not line: #checks for empty strings as document end marker
            output.writelines([str(article_number), '\t', file_theory, '\t', article_dictionary['PY'], '\t', article_dictionary['AU'], '\t', article_dictionary['TI'], '\t', article_dictionary['SO'], '\t', article_dictionary['AB'], '\n']) #outputs tab-separated file with article identifier, field, year, author, title, journal and abstract
            article_dictionary = {'AU' : '0', 'TI' : '0', 'AB' : '0', 'SO' : '0', 'PY' : '0'} #wipes the identifiers to prevent key errors
        ...
        if line[0] != ' ': #if it is not a continuation of a previous item type
        	item_code = line[0:2]  #then set the item type to the first 2 letters of the string. this stays over empty lines and solves the secondary author problems.
